...
const  log_prefix = "chinapost.js - "
/**
 * 阿里百炼 Provider
 * 使用 createOpenAICompatible 来支持 providerOptions
 * 参考: https://github.com/vercel/ai/issues/6037
 */
class ChinaPostClient extends BaseClient {
    constructor(config) {
        super(config);
        // 使用 createOpenAICompatible，name 必须设置为 'qwen' 才能使 providerOptions.qwen 生效
        this.china_post_model = createOpenAICompatible({
            name: CUSTOM_MODEL_CONFIG.CHINAPOST_QWEN25VL_MODELNAME,
            apiKey: this.apiKey,
    ...
    _getModel() {
        return this.china_post_model(this.model);
    }
    ...
    async chat(messages, options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`CHINAPOST QWEN2.5VL API 调用失败: ${response.status} ${errorText}`);
            }
            ...
            console.error('CHINAPOST QWEN2.5VL API 调用错误:', error);
            throw error;
        }
    }
...
module.exports = ChinaPostClient;
